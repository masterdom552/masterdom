...
public sealed class ActivatePolicyVersionCommandHandler : ICommandHandler<ActivatePolicyVersionCommand, ExecutionResult<PolicyAggregate>>
{
    private readonly IPolicyFrameworkApplicationService _applicationService;

    public ActivatePolicyVersionCommandHandler(IPolicyFrameworkApplicationService applicationService)
    {
        _applicationService = applicationService ?? throw new ArgumentNullException(nameof(applicationService));
    }

    public ExecutionResult<PolicyAggregate> Handle(ActivatePolicyVersionCommand command)
    {
        ArgumentNullException.ThrowIfNull(command);

        try
        {
            var policy = _applicationService.ActivatePolicyVersion(command);
