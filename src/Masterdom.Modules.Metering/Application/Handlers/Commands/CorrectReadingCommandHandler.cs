...
public sealed class CorrectReadingCommandHandler : ICommandHandler<CorrectReadingCommand, ExecutionResult<MeterAggregate>>
{
    private readonly IMeteringApplicationService _applicationService;

    public CorrectReadingCommandHandler(IMeteringApplicationService applicationService)
    {
        _applicationService = applicationService ?? throw new ArgumentNullException(nameof(applicationService));
    }

    public ExecutionResult<MeterAggregate> Handle(CorrectReadingCommand command)
    {
        ArgumentNullException.ThrowIfNull(command);

        try
        {
            var meter = _applicationService.CorrectReading(command);
