...
public sealed class ApproveReadingCommandHandler : ICommandHandler<ApproveReadingCommand, ExecutionResult<MeterAggregate>>
{
    private readonly IMeteringApplicationService _applicationService;

    public ApproveReadingCommandHandler(IMeteringApplicationService applicationService)
    {
        _applicationService = applicationService ?? throw new ArgumentNullException(nameof(applicationService));
    }

    public ExecutionResult<MeterAggregate> Handle(ApproveReadingCommand command)
    {
        ArgumentNullException.ThrowIfNull(command);

        try
        {
            var meter = _applicationService.ApproveReading(command);
