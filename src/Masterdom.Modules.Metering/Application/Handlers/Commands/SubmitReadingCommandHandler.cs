...
public sealed class SubmitReadingCommandHandler : ICommandHandler<SubmitReadingCommand, ExecutionResult<MeterAggregate>>
{
    private readonly IMeteringApplicationService _applicationService;

    public SubmitReadingCommandHandler(IMeteringApplicationService applicationService)
    {
        _applicationService = applicationService ?? throw new ArgumentNullException(nameof(applicationService));
    }

    public ExecutionResult<MeterAggregate> Handle(SubmitReadingCommand command)
    {
        ArgumentNullException.ThrowIfNull(command);

        try
        {
            var meter = _applicationService.SubmitReading(command);
