...
public sealed class PostPaymentJournalCommandHandler : ICommandHandler<PostPaymentJournalCommand, ExecutionResult<LedgerAggregate>>
{
    private readonly ILedgerApplicationService _applicationService;

    public PostPaymentJournalCommandHandler(ILedgerApplicationService applicationService)
    {
        _applicationService = applicationService ?? throw new ArgumentNullException(nameof(applicationService));
    }

    public ExecutionResult<LedgerAggregate> Handle(PostPaymentJournalCommand command)
    {
        ArgumentNullException.ThrowIfNull(command);

        try
        {
            var ledger = _applicationService.PostPaymentJournal(command);
