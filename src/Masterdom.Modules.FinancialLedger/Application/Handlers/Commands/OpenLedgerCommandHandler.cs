...
public sealed class OpenLedgerCommandHandler : ICommandHandler<OpenLedgerCommand, ExecutionResult<LedgerAggregate>>
{
    private readonly ILedgerApplicationService _applicationService;

    public OpenLedgerCommandHandler(ILedgerApplicationService applicationService)
    {
        _applicationService = applicationService ?? throw new ArgumentNullException(nameof(applicationService));
    }

    public ExecutionResult<LedgerAggregate> Handle(OpenLedgerCommand command)
    {
        ArgumentNullException.ThrowIfNull(command);

        try
        {
            var ledger = _applicationService.OpenLedger(command);
