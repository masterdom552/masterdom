...
public sealed class ReverseJournalCommandHandler : ICommandHandler<ReverseJournalCommand, ExecutionResult<LedgerAggregate>>
{
    private readonly ILedgerApplicationService _applicationService;

    public ReverseJournalCommandHandler(ILedgerApplicationService applicationService)
    {
        _applicationService = applicationService ?? throw new ArgumentNullException(nameof(applicationService));
    }

    public ExecutionResult<LedgerAggregate> Handle(ReverseJournalCommand command)
    {
        ArgumentNullException.ThrowIfNull(command);

        try
        {
            var ledger = _applicationService.ReverseJournal(command);
