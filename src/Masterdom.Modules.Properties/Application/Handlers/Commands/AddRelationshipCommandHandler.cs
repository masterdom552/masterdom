...
public sealed class AddRelationshipCommandHandler
    : ICommandHandler<AddRelationshipCommand, ExecutionResult<Property>>
{
    private readonly IPropertyApplicationService _applicationService;

    public AddRelationshipCommandHandler(IPropertyApplicationService applicationService)
    {
        _applicationService = applicationService ?? throw new ArgumentNullException(nameof(applicationService));
    }

    public ExecutionResult<Property> Handle(AddRelationshipCommand command)
    {
        ArgumentNullException.ThrowIfNull(command);

        try
        {
            var property = _applicationService.AddRelationship(command);
