...
public sealed class ChangeRemarksCommandHandler
    : ICommandHandler<ChangeRemarksCommand, ExecutionResult<Property>>
{
    private readonly IPropertyApplicationService _applicationService;

    public ChangeRemarksCommandHandler(IPropertyApplicationService applicationService)
    {
        _applicationService = applicationService ?? throw new ArgumentNullException(nameof(applicationService));
    }

    public ExecutionResult<Property> Handle(ChangeRemarksCommand command)
    {
        ArgumentNullException.ThrowIfNull(command);

        try
        {
            var property = _applicationService.ChangeRemarks(command);
