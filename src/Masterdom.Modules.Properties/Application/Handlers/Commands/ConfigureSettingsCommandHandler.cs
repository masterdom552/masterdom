...
public sealed class ConfigureSettingsCommandHandler
    : ICommandHandler<ConfigureSettingsCommand, ExecutionResult<Property>>
{
    private readonly IPropertyApplicationService _applicationService;

    public ConfigureSettingsCommandHandler(IPropertyApplicationService applicationService)
    {
        _applicationService = applicationService ?? throw new ArgumentNullException(nameof(applicationService));
    }

    public ExecutionResult<Property> Handle(ConfigureSettingsCommand command)
    {
        ArgumentNullException.ThrowIfNull(command);

        try
        {
            var property = _applicationService.ConfigureSettings(command);
