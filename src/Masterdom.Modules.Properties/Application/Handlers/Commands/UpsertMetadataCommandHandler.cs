...
public sealed class UpsertMetadataCommandHandler
    : ICommandHandler<UpsertMetadataCommand, ExecutionResult<Property>>
{
    private readonly IPropertyApplicationService _applicationService;

    public UpsertMetadataCommandHandler(IPropertyApplicationService applicationService)
    {
        _applicationService = applicationService ?? throw new ArgumentNullException(nameof(applicationService));
    }

    public ExecutionResult<Property> Handle(UpsertMetadataCommand command)
    {
        ArgumentNullException.ThrowIfNull(command);

        try
        {
            var property = _applicationService.UpsertMetadata(command);
