...
public sealed class SetDisplayOrderCommandHandler
    : ICommandHandler<SetDisplayOrderCommand, ExecutionResult<Property>>
{
    private readonly IPropertyApplicationService _applicationService;

    public SetDisplayOrderCommandHandler(IPropertyApplicationService applicationService)
    {
        _applicationService = applicationService ?? throw new ArgumentNullException(nameof(applicationService));
    }

    public ExecutionResult<Property> Handle(SetDisplayOrderCommand command)
    {
        ArgumentNullException.ThrowIfNull(command);

        try
        {
            var property = _applicationService.SetDisplayOrder(command);
