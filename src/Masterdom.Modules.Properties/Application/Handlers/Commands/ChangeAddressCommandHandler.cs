...
public sealed class ChangeAddressCommandHandler
    : ICommandHandler<ChangeAddressCommand, ExecutionResult<Property>>
{
    private readonly IPropertyApplicationService _applicationService;

    public ChangeAddressCommandHandler(IPropertyApplicationService applicationService)
    {
        _applicationService = applicationService ?? throw new ArgumentNullException(nameof(applicationService));
    }

    public ExecutionResult<Property> Handle(ChangeAddressCommand command)
    {
        ArgumentNullException.ThrowIfNull(command);

        try
        {
            var property = _applicationService.ChangeAddress(command);
