...
/// <summary>
/// Handles remove-party-role command orchestration.
/// </summary>
public sealed class RemovePartyRoleCommandHandler : ICommandHandler<RemovePartyRoleCommand, ExecutionResult<bool>>
{
    private readonly IPartyApplicationService _applicationService;

    public RemovePartyRoleCommandHandler(IPartyApplicationService applicationService)
    {
        _applicationService = applicationService ?? throw new ArgumentNullException(nameof(applicationService));
    }

    public ExecutionResult<bool> Handle(RemovePartyRoleCommand command)
    {
        ArgumentNullException.ThrowIfNull(command);

        try
        {
            var removed = _applicationService.RemovePartyRole(command);
            return removed
