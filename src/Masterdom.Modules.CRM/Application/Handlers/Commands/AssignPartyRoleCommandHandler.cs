...
/// <summary>
/// Handles assign-party-role command orchestration.
/// </summary>
public sealed class AssignPartyRoleCommandHandler : ICommandHandler<AssignPartyRoleCommand, ExecutionResult<Party>>
{
    private readonly IPartyApplicationService _applicationService;

    public AssignPartyRoleCommandHandler(IPartyApplicationService applicationService)
    {
        _applicationService = applicationService ?? throw new ArgumentNullException(nameof(applicationService));
    }

    public ExecutionResult<Party> Handle(AssignPartyRoleCommand command)
    {
        ArgumentNullException.ThrowIfNull(command);

        try
        {
            var party = _applicationService.AssignPartyRole(command);
