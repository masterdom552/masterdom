...
public sealed class ArchiveTenancyCommandHandler : ICommandHandler<ArchiveTenancyCommand, ExecutionResult<TenancyAggregate>>
{
    private readonly ITenancyApplicationService _applicationService;

    public ArchiveTenancyCommandHandler(ITenancyApplicationService applicationService)
    {
        _applicationService = applicationService ?? throw new ArgumentNullException(nameof(applicationService));
    }

    public ExecutionResult<TenancyAggregate> Handle(ArchiveTenancyCommand command)
    {
        ArgumentNullException.ThrowIfNull(command);

        try
        {
            var tenancy = _applicationService.ArchiveTenancy(command);
