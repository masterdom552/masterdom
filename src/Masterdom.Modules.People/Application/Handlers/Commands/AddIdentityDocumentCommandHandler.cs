...
/// <summary>
/// Handles add-identity-document command orchestration.
/// </summary>
public sealed class AddIdentityDocumentCommandHandler : ICommandHandler<AddIdentityDocumentCommand, ExecutionResult<Person>>
{
    private readonly IPersonApplicationService _applicationService;

    public AddIdentityDocumentCommandHandler(IPersonApplicationService applicationService)
    {
        _applicationService = applicationService ?? throw new ArgumentNullException(nameof(applicationService));
    }

    public ExecutionResult<Person> Handle(AddIdentityDocumentCommand command)
    {
        ArgumentNullException.ThrowIfNull(command);

        try
        {
            var person = _applicationService.AddIdentityDocument(command);
