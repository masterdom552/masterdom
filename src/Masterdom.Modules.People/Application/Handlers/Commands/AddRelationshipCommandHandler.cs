...
/// <summary>
/// Handles add-relationship command orchestration.
/// </summary>
public sealed class AddRelationshipCommandHandler : ICommandHandler<AddRelationshipCommand, ExecutionResult<Person>>
{
    private readonly IPersonApplicationService _applicationService;

    public AddRelationshipCommandHandler(IPersonApplicationService applicationService)
    {
        _applicationService = applicationService ?? throw new ArgumentNullException(nameof(applicationService));
    }

    public ExecutionResult<Person> Handle(AddRelationshipCommand command)
    {
        ArgumentNullException.ThrowIfNull(command);

        try
        {
            var person = _applicationService.AddRelationship(command);
